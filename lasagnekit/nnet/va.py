...
def decoder_term_gaussian(X, x_mean_hat, x_log_sigma_hat):
    ...
    x_hat = X - x_mean_hat
    log_x_given_z = -0.5 * \
        ((x_hat ** 2) / T.exp(2. * x_log_sigma_hat) + 2 * x_log_sigma_hat) - \
        0.5 * np.log(2 * np.pi)
...
def decoder_term_bernoulli(X, x_mean_hat):
    ...
    x_hat = -(X * T.nnet.softplus(-x_mean_hat) + (1 - X)
              * T.nnet.softplus(x_mean_hat))
...
def z_from_epsilon(epsilon, z_mean, z_log_sigma):
    ...
    shape = [z_mean.shape[0], 1] + [z_mean.shape[i]
                                    for i in range(1, z_mean.ndim)]
    z_mean_ = z_mean.reshape(shape)

    shape = [z_log_sigma.shape[0], 1] + [z_log_sigma.shape[i]
                                         for i in range(1, z_log_sigma.ndim)]
...
Categorical = dict(
    mean_param_index=0,
    nonlinearity=nonlinearities.linear,
    decoder_loss_function=lambda X, X_params: decoder_term_categorical(
        X, X_params[0]),
...
class VariationalAutoencoder(object):
    ...
    def build(self, X):
        ...
        self._prior_z_mean = theano.shared(
            np.zeros(z_shape, dtype=theano.config.floatX))
        self._prior_z_log_sigma = theano.shared(
            np.zeros(z_shape, dtype=theano.config.floatX))

        nb_batches = easy.get_nb_batches(
            len(X), self.batch_optimizer.batch_size)

        if isinstance(self.X_type, T.TensorVariable):
            X_batch = self.X_type
        else:
            X_batch = self.X_type('X_batch')

        if isinstance(self.Z_type, T.TensorVariable):
            Z_batch = self.Z_type
        else:
        ...
        self.encode_function = theano.function(
            [X_batch], self.nnet_x_to_z.get_output(X_batch, determnistic=True))

        nb_z_samples = (self.nb_z_samples
                        if self.nb_z_samples is not None
                        else 1)

        epsilon_shape = [z_mean.shape[0], nb_z_samples] + \
            [z_mean.shape[i] for i in range(1, z_mean.ndim)]
        epsilon = self.rng.normal(epsilon_shape)

        z = z_from_epsilon(epsilon, z_mean, z_log_sigma)

        z_for_nnet_shape = [z.shape[0] * z.shape[1]] + \
            [z.shape[i] for i in range(2, z.ndim)]
        z_for_nnet = z.reshape(z_for_nnet_shape)

        # x_mean, x_log_sigma for example
        X_params = self.nnet_z_to_x.get_output(z_for_nnet)
        for i in range(len(X_params)):
            p = X_params[i]
            shape = [X_batch.shape[0], z.shape[1]] + [p.shape[j]
                                                      for j in range(1, p.ndim)]
            X_params[i] = p.reshape(shape)
        # loss
        x_mean = X_params[self.input_type.get("mean_param_index")]

        rec_error = ((X_batch - self.input_type.get("nonlinearity")(x_mean.mean(axis=1)))**2).sum(axis=1).mean()
        self.reconstruction_error_function = theano.function(
                [X_batch],
                rec_error,
                givens={Z_batch: z_for_nnet} # just in case  ( :) )
        )

        shape = [X_batch.shape[0], 1] + [X_batch.shape[i]
                                         for i in range(1, X_batch.ndim)]
        ...
        self.get_likelihood_lower_bound = theano.function(
                [X_batch],
                loss,
                givens={Z_batch: z_for_nnet}) # just in case ( :) )
        loss = loss.mean(axis=0)
        # the decode function
        X_params_from_z = self.nnet_z_to_x.get_output(
            Z_batch, deterministic=True)

        ind_mean = self.input_type.get("mean_param_index")
        X_params_from_z[ind_mean] = self.input_type.get(
            "nonlinearity")(X_params_from_z[ind_mean])
        ...
        log_p_x_given_z = self.input_type.get(
            "decoder_loss_function")(X_batch_for_loss, X_params)

        z_mean_shape = [z_mean.shape[0], 1] + [z_mean.shape[i]
                                               for i in range(1, z_mean.ndim)]
        z_log_sigma_shape = [z_log_sigma.shape[
            0], 1] + [z_log_sigma.shape[i] for i in range(1, z_log_sigma.ndim)]
        log_q_z_given_x = decoder_term_gaussian(
            z, z_mean.reshape(z_mean_shape), z_log_sigma.reshape(z_log_sigma_shape))
        log_p_z = decoder_term_gaussian(
            z, self._prior_z_mean, self._prior_z_log_sigma)
        log_likelihood_approximation = - \
            (easy.log_sum_exp(
                log_p_z + log_p_x_given_z - log_q_z_given_x, axis=1) - T.log(nb_z_samples))
        self.log_likelihood_approximation_function = theano.function(
                [X_batch],
                log_likelihood_approximation,
                givens={Z_batch: z_for_nnet})

        # Parameters and optimization
        all_params = (self.nnet_x_to_z.get_all_params(trainable=True) +
                      self.nnet_z_to_x.get_all_params(trainable=True)
                      + [self._prior_z_mean, self._prior_z_log_sigma])
        ...
        if self.batch_optimizer.whole_dataset_in_device == True:
            ...
            iter_update_batch = theano.function(
                ...
                givens={
                    ...
                    Z_batch: z_for_nnet[batch_slice]
                },
            )
        else:
            iter_update = theano.function(
                ...
                updates=updates,
                givens={
                    Z_batch: z_for_nnet # just in case
                }
            )
    ...
    def sample(self, nb, only_means=False, bias=0):
        ...
        Z = np.random.multivariate_normal(prior_z_mean,
                                          np.diag(np.exp(prior_z_log_sigma - bias)),
                                          size=nb)
